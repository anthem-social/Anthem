...
// encryption
const encSecret = crypto.createHash('sha256').update(process.env.ENCRYPTION_SECRET).digest(); // Ensure key is 32 bytes
const encMethod = process.env.ENCRYPTION_METHOD || "aes-256-ctr";
const iv = crypto.randomBytes(16); // Generate a random IV

const encrypt = (text) => {
    const aes = crypto.createCipheriv(encMethod, Buffer.from(encSecret), iv);
...
const decrypt = (text) => {
    ...
    const iv = Buffer.from(textParts.shift(), 'hex'); // Extract IV from the encrypted text
    const encryptedText = textParts.join(':');
    const aes = crypto.createDecipheriv(encMethod, Buffer.from(encSecret), iv);
    let decrypted = aes.update(encryptedText, 'hex', 'utf8');
