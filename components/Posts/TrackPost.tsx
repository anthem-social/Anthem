...
import { saveTrack, unsaveTrack } from "@/api/spotify";
...
export function TrackPost(props: Props) {
  ...
  const toggleSave = async (uri: string) => {
    var previous = saved;

    if (previous) {
      var unsave = await unsaveTrack(uri);

      if (unsave.IsFailure) {
        console.log(unsave.ErrorMessage);
        return;
      }
      ...
      var save = await saveTrack(uri);

      if (save.IsFailure) {
        console.log(save.ErrorMessage);
        return;
      }
    ...
    <View style={styles.container}>
      <View style={[styles.row, { paddingHorizontal: 4, paddingVertical: 6, alignItems: "center" }]}>
        ...
        <View style={styles.col}>
          <TouchableOpacity style={styles.save} onPress={() => toggleSave(props.track.uri)}>
